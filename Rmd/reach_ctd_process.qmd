...
ctd_rm <- "REACH003_S10_10|REACH004_S15_7|REACH004_S21_2"

# remove cast with only upcast from REACH003, stn 10 cast 10
ctd_path <- ctd_path[which(!str_detect(ctd_path, ctd_rm))] %T>% 
  print()
```
...
(as.data.frame(test_ctd@data) %>%
  ggplot(aes(y = depth, x = temperature)) +
  geom_path() +
    geom_point(aes(color = scan), alpha = 0.5) +
  scale_y_continuous(transform = "reverse") +
    scale_color_viridis_c()) %>%
  plotly::ggplotly() 
# plot(test_ctd, which = "temperature", type = "l") 
...
test_dec <- ctdDecimate(test_trim, p = 0.25, e = 2)
plot(test_dec)
# plot(test_dec2)
...
ctd_all <- 
  ...
  \(.x) {
    ...
    if (str_detect(.x, "REACH002")) {
    station <- 
      basename(.x) %>%
      str_remove("_DatCnv_processed") %>%
...
for (sel_stn in sel_stn1) {
  ...
  if (length(sel_stn_num) < 2) {
    message("Not enough stations to plot, skipping.")
    next
  }
...
ctd_section <- as.section(ctd_all[str_detect(names(ctd_all), "S15|S16|S17")][c(1,2,3)])
plot(ctd_section)
plot(
  ctd_section, 
  which = "temperature",
...
ctd_section <- as.section(ctd_all[str_detect(names(ctd_all), "S08|GR|S09|S10|S11")][c(2,1,3,4,5)])
plot(ctd_section)
plot(
  ctd_section, 
  which = "temperature",
...
ctd_section <- as.section(ctd_all[str_detect(names(ctd_all), "S17|S14|S10|S07|S22|S01")][c(1,2,3,4,5,6)])
plot(ctd_section)
plot(
  ctd_section, 
  which = "temperature",
  ...
  # zcol = oce::oceColorsTemperature
  # zcol = rev(ODV_colours)
  zcol = rev(colorRampPalette(ODV_colours)(100))
  )
plot(
  ctd_section, 
  which = "salinity",
...
# Save Envrionmental Plots

Box: "~Box/IMaRS_Projects/FLRACEP V (REACH) project/data/dataProcessing_phase1/reach_ctd"

```{r}
test <- 
  list(
    temp = c("temperature", "Temperature (°C)"),
...
print(ggplot(ctd_tib, aes(y = depth, color = station)) +
  ...
  # scale_color_manual(values = colorRampPalette(ODV_colours)(18)) +
  scale_color_manual(values = colorRampPalette(ODV_colours)(23)) +
  # lemon::geom_pointline(aes(x = temperature), linewidth = rel(1), size = rel(0.1)) +
  # geom_path(aes(x = get(column[[1]][1])), size = rel(0.5)) +
  geom_path(aes(x = get(column[[1]][1])), linewidth = rel(0.5)) +
  theme_bw() +
  labs(
    x = column[[1]][2],
    y = "Depth [db]",
