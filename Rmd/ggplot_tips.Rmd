...
```{r}
# dat_biigle_prct_filt <-
#   dat_biigle_prct %>%
#   filter(
#     !is.na(plot_wigmali) 
#     & !is.nan(plot_wigmali) 
#     & !is.nan(prcnt_per_img)
#     ) %>%
#   mutate(
#     plot_wigmali = 
#       fct_reorder(
...
plt_box <-
  ...
  geom_tile(
    data =
      ~ .x %>%
      distinct(plot_wigmali, site_name) %>%
      complete(site_name, plot_wigmali) %>%
      mutate(y = if_else(str_detect(plot_wigmali, rec_factor), 1, NA)),
    aes(
      x = plot_wigmali,
      y = y,
      height = Inf,
