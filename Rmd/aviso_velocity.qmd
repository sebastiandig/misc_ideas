...
various transects and are named after the `Line Names` inherent to them. 
...
### Set `ffmpeg` location for CMS computer

```{r cms-computer-location}
if (Sys.getenv("USERNAME") == "sebastian15") {
  ffmpeg_path <- here("C:\\", "Users", "sebastian15", "AppData", "Roaming", 
...
aviso_path <-
  aviso_path_full %>%
  Filter(\(.x) str_detect(.x, year_to_run), .) %T>% 
  {
    .[] %>%
