...
```{r load-multi}
avios_days <- avios_load(ssh_path, .by = 7)

ssh_grid_tibble <- avios_days$tibble
dat_grid        <- avios_days$gridded
...
trasect_plt <- 
  dist_from_shore %>%
  ggplot(aes(x = dist, y = vel_mag, color = as.factor(l1))) +
...
plt_mag_vel_vect <- 
  ...
  # velocity pixels
  geom_tile(data = ssh_tile, aes(x = lon, y = lat, fill = vel_mag)) +
