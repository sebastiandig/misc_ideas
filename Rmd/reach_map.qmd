...
loc_dat <- 
  ...
  readxl::read_xlsx(.name_repair = janitor::make_clean_names, sheet = "Times (3)") %>%
  janitor::remove_empty("cols") %>%
  drop_na(latitude_decimal_degree, longitude_decimal_degree) %T>%
  # distinct(longitude_decimal_degree, latitude_decimal_degree, .keep_all = TRUE) %T>% 
  print() %>%
  select(waypoint, "lat" = latitude_decimal_degree, "long" = longitude_decimal_degree, depth_m, departure_date_time_local) %>%
  mutate(
    long = -long,
...
loc_map_pts <-
  ...
  geom_point(
    data = loc_dat,
    aes(x = long, y = lat, fill = fct_reorder(format(date, "%B %d"), date)),
    size = 3,
    shape = 21
    # show.legend = FALSE
  ) +
