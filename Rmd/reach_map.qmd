...
loc_other <- 
  ...
  dir_ls(regexp = "(?i)schedule") %>%
  str_subset("~", negate = TRUE) %>% 
  read_xlsx(.name_repair = make_clean_names, sheet = sheet_names[1]) %>%
  remove_empty("cols") %>%
  remove_empty("rows") %T>%
  # drop_na(latitude_decimal_degree, longitude_decimal_degree) %T>%
  print() %>%
  select(
    waypoint, 
    "lat" = lat_in_degree, "long" = lon_in_degree, 
    ) %>%
  mutate(
    waypoint = str_remove(waypoint, "_(D|R|S|cal|drop)")
    ) %>%
  filter_out(
    waypoint %in% str_remove(loc_dat$waypoint, "_(D|R|S|cal|drop)") |
    waypoint %in% str_remove(loc_dat$waypoint, "S\\d{2}/") |
    str_detect(waypoint, paste(filter(loc_dat, str_detect(waypoint, "C"))$waypoint, collapse = "|"))
  ) %>%
  distinct() %T>%
  print() 
```
...
```
## 2.4 Load Passive Acoustic Monitoring (PAM) Locations
from Chad Lembke 
In reference to our locations

```{r}
# pam_points <- rstudioapi::selectFile() 
pam_points <-
  here("data", "raw", "SEFSC Acoustic Mooring Sites") %>%
  dir_ls(regexp = "\\.shp$") %>%
  st_read()
```
...
leaflet::leaflet() %>%
  addTiles() %>%
  addMarkers(loc_dat$long, loc_dat$lat, label = loc_dat$waypoint) %>%
  addCircleMarkers(pam_points$Longitude, pam_points$Latitude,
             label = pam_points$SiteID)
```
...
bathy_contour_label <- 
  bathy %>%
  filter(
    between(x, bounds[1] + 0.1, bounds[2] - 0.1),
    between(y, bounds[3] + 0.1, bounds[4] - 0.1),
  ) %>%
  contour_labels(upper_lower = "upper", n_per_level = 1)
...
long_lat_lag <- 
  ...
  mutate(
    ...
    waypoint = str_remove(waypoint, "_(R|S|D)"),
    description = str_remove(description, "Recover|Deploy")
  ) %>%
  select(-te, -t2) %T>% 
  print()
```
...
"CTD/Bongo and Tucker/Subsurface",     22,      "#ffd", "black",
           ...
           "Surface Buoy Service",     25,      "blue", "black",
                         "Missed",      4,       "red",   "red",
  )
