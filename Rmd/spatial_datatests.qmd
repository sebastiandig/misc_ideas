...
### Load Animate Packages and Set Path to FFMPEG

Will need to have downloaded `ffmpeg` 
...
plt_mag_vel <- 
  ...
  geom_point(data = vel_ext_transect, aes(x = x,  y = y), color = "blue",
             size = 0.5) +
  geom_point(data = filter(vel_ext_transect, vel_mag >= 0.5), 
             aes(x = x,  y = y), color = "green",
             size = 0.5) +
  labs(x = NULL, y = NULL) +
...
# anim_save("mag_vel.gif", plt_mag_vel, path = animate_save)
anim_save(
  file      = glue("mag_vel_{date_suffix}.mp4"), 
  animation = plt_mag_vel, 
  path      = animate_save, 
  renderer  = ffmpeg_renderer(format = "mp4", ffmpeg = ffmpeg_path), 
  height = 800, 
  width  = 800,
...
# plt_mag_vel_vect
# anim_save("mag_vel_vect.gif", plt_mag_vel_vect, path = animate_save)
anim_save(
  file      = glue("mag_vel_vect_{date_suffix}.mp4"), 
