...
```{r}
# create multiday raster
vector("list", length(nc_date))

multi_rast <- list(
  rast(dat_grid[[1]], crs = "EPSG:4326"),
  rast(dat_grid[[2]], crs = "EPSG:4326"),
  rast(dat_grid[[3]], crs = "EPSG:4326") 
) %>%
  rast()
...
multi_rast <- 
  ...
  rast()
plot(multi_rast)
all.equal(multi_rast, multi_rast2)
...
n_layers <- nlyr(multi_rast) / length(nc_date)
  
terra::plot
(dat_grid[[3]])
test <- rast(dat_grid[[3]], crs = "EPSG:4326") 

time(test) <- rep(as_date(names(dat_grid)[[3]]), nlyr(test))

time(multi_rast) <-
  c(
    rep(as_date(names(dat_grid)[[1]]), n_layers),
    rep(as_date(names(dat_grid)[[2]]), n_layers),
    rep(as_date(names(dat_grid)[[3]]), n_layers)
  )
...
slice_by <- 5
ggplot() +
  geom_tile(data = ssh_grid_tibble, aes(x = lon, y = lat, fill = adt)) +
  geom_sf(data = coast) +
  scale_fill_viridis_b() +
  geom_sf(data = linestring, color = "red")  +
  geom_point(data = vel_ext_transect, aes(x = x,  y = y), color = "blue") +
