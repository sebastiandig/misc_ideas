...
plt +
  geom_sf(data = polyline$finished, color = "red")

vel_extract <- terra::extract(rssh_grid_rast, polyline$finished, xy = TRUE)
...
left_join(
  ...
  rowwise() %>%
  mutate(
    dist = geosphere::distGeo(c(x.x, y.x), c(x.y, y.y)) / 1000
  )
