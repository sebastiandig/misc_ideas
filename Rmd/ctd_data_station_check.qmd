...
all_cruise_info$info %>%
  separate_wider_delim(title, delim = ", ", names_sep = "_", too_many = "debug") %T>%
  print() %>%
  mutate(
    .before = title_5,
    # title_5 = str_replace(title_5, " 82W", " 82.0W"),
    # lat_lon = parzer::parse_llstr(title_5),
    rn = str_to_upper(title_3),
    rn = str_remove(rn, " "),
    rn = str_replace(rn, "WS20278", "WS20279"),
    rn = str_replace(rn, "WS21277", "WS21278"),
    rn = str_replace(rn, "WS23011", "WS23010"), # weird
    rn = str_remove_all(rn, "_AUG_2011_|2022_08_WEATHERBIRD_SMITH_CTD_|KELBLE_|HEX|SFP_"),
    # rn = str_remove(rn, "KELBLE_"),
    rn = str_remove_all(rn, title_2),
    rn2 = rn,
    rn = str_remove(rn, "^_-|^_{1,2}"),
    rn = str_remove(rn, "^\\."),
    rn = str_remove(rn, "\\s?(^STA|^STN)\\.?_?"),
    rn = str_remove(rn, "^0{1,2}"),
    rn = str_replace(rn ,"(UK|EK|Z\\d{2})(_|-)(.*)", "\\1\\3"),
    rn = if_else(str_detect(rn, "LK|^21"), "21LK", rn),
  ) %>%
  filter(str_detect(rn2, "LK|21"))
  # unnest(lat_lon) %>%
  arrange(rn) %>%
  distinct(rn) %$% 
  rn
