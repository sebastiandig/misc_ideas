...
```{r libs}
# fixes an issue with "time_elapse" column where converts to numeric
# instead of datatime
set_units2 <- function (temp_t, dds) {
  for (i in seq(3, (nrow(dds) - 2))) {
    var_type <- dds[i, 1]
    var_name <- dds[i, 2]
    var_name <- substr(var_name, 1, nchar(var_name) - 1)
    if (var_name == "time" | var_name == "time_elapsed") {
...
assignInNamespace("set_units", set_units2, ns = "rerddap")
rerddap:::set_units
```
...
all_cruise_info <-
  ed_search_adv(
    # query     = "Walton Smith",
    query     = "SFER ",
    maxTime   = "2026-06-01T01:00:00Z",
    page_size = 10000
  )

all_cruise_ids <-
  str_extract(all_cruise_info$info$title, "(WS|SAV|WB|H|HG|SV)\\d{3,5}") %>%
  unique()
...
cruis_id <-
  ...
  separate_wider_delim(title, delim = ", ", names_sep = "_") %>%
  mutate(
    # title_2 = str_replace(title_2, "(57)_(\\d)", "\\1.\\2"),
    title_2 = str_replace(title_2, "_(57|22|15|21)_(\\d)", "_\\1.\\2"),
    title_2 = str_replace(title_2, "(_\\d{1,2})_(5|75)$", "\\1.\\2"),
    title_2 = str_replace(title_2, " cast ", "_"),
  ) %>%
  separate_wider_delim(title_2, delim = "_", names_sep = "_", too_few = "align_start", cols_remove = FALSE) %>% 
  print() %>%
  mutate(
    title_2_title_2 = str_remove(title_2_title_2, title_2_1),
    title_2_title_2 = str_remove(title_2_title_2, title_2_2),
    title_2_title_2 = str_remove(title_2_title_2, title_2_3),
    title_2_title_2 = str_remove(title_2_title_2, "___"),
  ) #%>%
  # distinct(title_2_4, title_2_5, title_2_title_2) %>%
  # arrange(title_2_title_2)
  # filter(str_detect(title_2_title_2, "__"))
  # filter(str_detect(dataset_id, "_2$"))
  # filter_out(is.na(title_2_5))
 #  %T>%
 #  print() %>%
 #  mutate(
 #    .before = title_4,
 #    title_5 = str_replace(title_4, " 82W", " 82.0W"),
 #    lat_lon = parse_llstr(title_5),
 #    station = str_to_upper(title_2),
...
```{r}
dir_create(here("data", "raw", "raw_ctd_data", unique(cruis_id$title_2_3)))
...
cruis_id %>% 
  mutate(
    .after = 3,
    .by = title_2_3,
    cruise_num = cur_group_id(),
    station_num = row_number(),
    station_max = max(station_num)
  ) %>% 
  mutate(
    .after = cruise_num,
    cruise_max = max(cruise_num)
  ) %$%
  pwalk(
    list(
    .x = title_2_3, 
    ...
    \(.x, .y, cr_n, cr_max, stn_n, stn_max) {
      message(paste("\n--\n"))
      message(glue("Cruise ID: {.x} ({cr_n} of {cr_max})"))
      message(glue("Dataset ID: {.y} ({stn_n} of {stn_max})"))

      file_name <- here("data", "raw", "ctd", .x, paste0(.y, ".csv"))
      if (file_exists(file_name)) {
        message("File exist. Skipping!")
        return()
      }

      message("Dowloading file!")
      # get data

      out <- info(.y)
      ctd_raw <- suppressWarnings(tabledap(out, url = eurl(), store = disk()))
      write_csv(as.data.frame(ctd_raw), file_name)
    }
  )
...
all_dat <- 
  here("data", "raw") %>% 
  dir_ls(regexp = "ctd") %>% 
  dir_ls(regexp = "csv", recurse = 1) %>%
  tibble(x = .) %>% 
  # slice(c(1:5, 110:118)) %>%
  mutate(
    data = map(
      x, 
      \(.x) read_csv(.x, n_max = 1, show_col_types = FALSE) %>% 
        mutate(
...
# save_tops <- rstudioapi::selectDirectory()
save_tops <- "C:/Users/sebastian15/Box/sebastian15"

all_dat

unnest(all_dat, data) %>% 
  write_csv(here(save_tops, "ctd_first.csv"), na = "")
```
...
ctd_cast <- 
  cruis_id %>%
  # filter(str_detect(title_2, "SV18067_54")) %>%
  # filter(str_detect(title_2, "SV18067_17")) %>%
  # filter(str_detect(title_2, "SV18067_45")) %>%
  filter(str_detect(dataset_id, "WS1116_19")) %>%
  pull(dataset_id) %T>% 
  print()
...
# visdat::vis_miss(ctd_data)
ctd_obj2 <- as.data.frame(ctd_data) %>%
  janitor::remove_empty("cols") %>%
  select(!matches("qc_(agg|gap|syntax|location|gross_range|flat_line|spike|rate_of|climatology|decreasing_radiance)")) %$% 
   as.ctd(
        salinity    = sea_water_salinity,
        temperature = sea_water_temperature,
        pressure    = sea_water_pressure,
        station     = paste("Staiton", station[1]),
        cruise      = cruiseID,
        longitude   = longitude,
        latitude    = latitude,
        time        = time
...
```{r}
trimmed_cast <- ctdTrim(ctd_obj2)
decimated_cast <- ctdDecimate(trimmed_cast, p = 0.1, na.rm = TRUE)
