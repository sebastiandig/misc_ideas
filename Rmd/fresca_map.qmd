...
ctd_data <- 
  ...
  filter(!str_detect(station, "16|10|15\\.5")) %>%
  arrange(lon) %>%
  mutate(
    x2 = dplyr::lag(lon),
    y2 = dplyr::lag(lat),
...
topo_dist_poly <-
  bind_rows(
    slice(topo_dist, 1) %>%
      ...
      mutate(alt_smooth = min(topo_dist$altitude, na.rm = TRUE) - 1),
    topo_dist,
    slice_tail(topo_dist, n = 1) %>%
      # mutate(alt_smooth = min(topo_dist$alt_smooth, na.rm = TRUE) - 1)
      mutate(alt_smooth = min(topo_dist$altitude, na.rm = TRUE) - 1)
...
ggplot(topo_dist_poly) +
  geom_polygon(fill = "gray80", aes(x = dist_cum, y = alt_smooth), color = "blue") +
  geom_line(
...
oce::plot(
  ...
  zcol = rev(colorRampPalette(ODV_colours)(120))
  # zbreaks = seq(25, 35, 1)
  )
