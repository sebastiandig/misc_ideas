...
dir_data_location <- 
  here(
    "E:\\", "google_drive", ".shortcut-targets-by-id", 
    "1E3fmKuWxmqhFspHhqvbFEI5qKGGKJgnk", "SFER Ship Exported Data", 
    "WS21278_ShipExports"
    ) %>%
  dir_ls(regexp = "(?i)vids", type = "directory") %>%
  dir_ls() %T>% 
  print()
...
vtg_load <- 
  ...
  print() %>%
  hablar::retype(-contains(c("time", "date"))) %>%
...
wind_avg <- 
  ...
  mutate(
    .keep = "unused",
    rel_sines    = sin(NISTdegTOradian(relative_dir)),
    rel_cosines  = cos(NISTdegTOradian(relative_dir)),
    true_sines   = sin(NISTdegTOradian(true_dir)),
    true_cosines = cos(NISTdegTOradian(true_dir)),
  ) %T>%
  print() %>% 
  summarise(
    .by = c(1, 3),
...
merge_dat %>%
  ...
  arrange(date)
hdt_load %>%
  distinct(computer_date) %>%
  arrange(computer_date)
```
...
filt_names <- paste(paste("DisOrgMat",
"Fluorometer",
"MicroTSG1",
"PortRMYoung",
"POSMV",
...
"StarboardRMYoung",
"Turner C3 Primary",
"Turner C3 Secondary", sep = "$|"), "Turner" , sep = "|")

# after WS21151_ShipExports went to Turner C3
